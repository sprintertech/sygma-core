...
import (
	...
	"github.com/ChainSafe/sygma-core/chains/evm/config"
...
func (s *NewEVMConfigTestSuite) Test_FailedDecode() {
	_, err := config.NewEVMConfig(map[string]interface{}{
		"gasLimit": "invalid",
	})
...
func (s *NewEVMConfigTestSuite) Test_ValidConfig() {
	...
	actualConfig, err := config.NewEVMConfig(rawConfig)

	id := new(uint8)
	*id = 1
	s.Nil(err)
	s.Equal(*actualConfig, config.EVMConfig{
